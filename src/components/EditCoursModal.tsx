...
function EditCoursModal({visible, showModal, coursToEdit}:any) {
    ...
    function handleAddFiliere(filiersIds:number[]) {
      console.log({filiersIds})
        setCours((cours:any) => ({...cours,filiers:filiersIds.map((id:any) =>({id:id.value}))}))
      }
    const tailLayout = { 
  ...
  const handleEdit = () => {
    ...
    dispatch(editOne("cours",cours.id,cours))
    showModal(false)
  }
    return (
        <Modal
